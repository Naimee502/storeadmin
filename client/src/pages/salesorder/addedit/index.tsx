...
const AddEditSalesOrder = () => {
  ...
  const { addSalesOrderMutation, editSalesOrderMutation } = useSalesOrderMutations();
  ...
  const customerAccounts = accountData?.getAccounts?.filter((acc: any) => isCustomerParty(acc.type)) || [];
  ...
  const salesorderFormPermissions = useAppSelector(state => state.permissions.permissions?.formPermissions?.salesorder || {});
  const isFieldEnabled = (fieldId: string) => salesorderFormPermissions[fieldId] !== false;
  ...
  useEffect(() => {
    if (isEdit && data?.getSalesOrderById) {
      const order = data.getSalesOrderById;
      setPaymentType(order.paymenttype || "");
      setPartyAccount({
        id: order.partyacc?.id || "",
        accountname: order.partyacc?.accountname || "",
        mobile: order.partyacc?.mobile || "",
        // Note: These might not be populated in basic ref, may need account query refinement if needed
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit && id) {
        await editSalesOrderMutation({ variables: { id, input } });
        dispatch(showMessage({ message: "Order updated successfully", type: "success" }));
      } else {
        await addSalesOrderMutation({ variables: { input } });
        dispatch(showMessage({ message: "Order added successfully", type: "success" }));
      }
      navigate(-1);
    } catch (error: any) {
      ...
      <div className="w-full px-2 sm:px-6 pt-4 pb-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <fieldset className="border rounded-xl p-4 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              {isFieldEnabled("partyacc") && (<FormField
                ...
                onChange={(e) => {
                  const acc = customerAccounts.find((a: any) => a.id === e.target.value);
          ...
          <div className="flex gap-4 justify-end">
            ...
            <Button type="submit" variant="outline">Save Order</Button>
          </div>
        </form>
