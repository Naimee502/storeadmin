...
// (server/src/graphql/resolvers/salesorder/index.ts). Lets the cart preview
// the delivery/handling/COD charges an order will pick up BEFORE it's
// placed, so what the user sees matches what addSalesOrder actually charges.
// Display-only: the cart still sends its original (charge-free) totalamount,
// the server adds these same charges on its own at order-creation time.
